...
namespace ThirdPersonShooter.Entities.Player
{
    public class PlayerMotor : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            if(input.normalized.magnitude != 0) rigidbody.AddForce((camera.forward * input.y + camera.right * input.x) * player.Stats.Speed, ForceMode.Impulse);
            
            if(animator) animator.SetFloat(speedID, Mathf.Clamp01(rigidbody.velocity.magnitude));

            if (Mathf.Approximately(input.normalized.magnitude, 1)) render.forward = Vector3.SmoothDamp(render.forward, camera.forward, ref dampingVelocity, alignmentDamping);
        }
